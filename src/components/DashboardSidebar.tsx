...
const SidebarItem: React.FC<SidebarItemProps> = ({ icon, label, href, active, children }) => {
      ...
      <div>
        <button
          ...
          className={cn(
            ...
            active ? "text-background bg-gradient-to-r from-primary to-secondary shadow-lg" : "text-foreground/80 hover:bg-muted/50 hover:text-primary"
          )}
        >
          <div className="flex items-center">
            <div className={cn("p-1.5 rounded-lg", active ? "bg-background/20 text-background" : "bg-gradient-to-r from-primary/10 to-secondary/10 text-primary")}>
              {icon}
            </div>
            <span className="ml-3 flex-1 text-left">{label}</span>
          </div>
          {expanded ? <ChevronDown size={18} className={active ? "text-background/80" : "text-foreground/60"} /> : <ChevronRight size={18} className={active ? "text-background/80" : "text-foreground/60"} />}
        </button>
              ...
              {children.map((child, i) => (
                <Link
                  ...
                  className={cn(
                    ...
                    location.pathname === child.href ? "text-primary font-medium" : "text-foreground/70 hover:text-primary"
                  )}
                >
...
const DashboardSidebar: React.FC = () => {
  ...
  return (
    <>
      ...
      <button className="sm:hidden fixed top-4 left-4 z-50 p-2 bg-primary/80 rounded-lg text-white" onClick={() => setIsOpen(true)}>
        <Menu size={20} />
      </button>
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            <div className="p-4">
              <button className="mb-4 p-2 rounded-lg bg-muted/50 text-foreground" onClick={() => setIsOpen(false)}>
                <X size={20} />
              </button>
              <SidebarContent profile={profile} navItems={navItems} location={location} handleLogout={handleLogout} />
            </div>
...
const SidebarContent: React.FC<SidebarContentProps> = ({ profile, navItems, location, handleLogout }) => (
  <div>
    {/* Logo Agrosub avec $ */}
    <motion.div className="flex items-center mb-8" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }}>
      ...
      <motion.span className="ml-3 text-xl sm:text-2xl md:text-3xl font-extrabold bg-clip-text text-transparent bg-gradient-to-r from-primary to-secondary" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }}>
        Agrosub
      </motion.span>
    </motion.div>
    ...
    {/* Profil utilisateur */}
    {profile && (
      <motion.div className="mb-6 p-4 bg-gradient-to-b from-muted/10 to-background rounded-xl border border-border shadow-sm" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
        <div className="flex items-center mb-3">
          {profile.avatar_url ? (
            <img src={profile.avatar_url} alt={profile.name} className="w-10 h-10 rounded-full object-cover border-2 border-primary" />
          ) : (
        ...
        <Link to="/mes-dossiers/nouveau" className="flex items-center text-sm font-medium transition-colors group">
    ...
    <nav className="space-y-1">
      <AnimatePresence>
        {navItems.map((item: any, i: number) => (
          <motion.div key={i} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 * (i + 1) }} className="mb-1">
            <SidebarItem
              icon={item.icon}
              label={item.label}
              href={item.href}
              active={location.pathname === item.href || (item.children && item.children.some((child: any) => location.pathname === child.href))}
    ...
    {/* Déconnexion */}
    <motion.button onClick={handleLogout} className="flex items-center w-full py-3 px-4 rounded-xl text-sm font-medium text-muted-foreground hover:bg-muted/50 hover:text-destructive transition-colors group mt-6" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.8 }}>
